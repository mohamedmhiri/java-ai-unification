...
public class Unification {
	public String unifier(List<String> expr1,List<String> expr2) {
			...
			//r�cup�rer le premier element de la premi�re expression
			String f1 = expr1.get(0);
			//sauvegarder les termes non trait�s de la premi�re expression
			expr1.remove(0);
			List<String> t1 = expr1;
			//r�cup�rer le premier element de la deuxi�me expression
			String f2 = expr2.get(0);
			//sauvegarder les termes non trait�s de la deuxi�me expression
			expr2.remove(0);
			...
			e2.add(f2);
			//unifier les t�tes de deux expressions
			String z1 = unifier(e1, e2);
			//echec d'unification
			if(z1.equalsIgnoreCase("echec"))
			return "echec";
			//Application des changements sur les termes non trait�s des expressions
			List<String> g1 = changer(t1, z1);
			List<String> g2 = changer(t2, z1);
			//unifier les termes non trait�s de deux expressions
...
public List<String> extractExpression(String e) {
		...
		while (!(expression.equals(""))) {
			...
			// il y a un seul �l�ment  soit une constante soit une variable
			if (!(expression.contains(","))) {
				...
				if (!(expression.substring(0, expression.indexOf(",")).contains("("))) {
					// On ajoute l'�l�ment dans la liste
					liste.add(expression.substring(0, expression.indexOf(",")));
					//System.out.println("ajout d'une constante ou variable");
					System.out.println("expression ajoute dans la liste "+expression.substring(0, expression.indexOf(",")));
				
				 // on enl�ve l element ajoute de la chaine
				...
				// Si le premier element de l'expression est une fonction
					...
					//Le cas o� le dernier �l�ment de la liste est une fonction 
					if (indexFin == expression.length() - 1) {
						expression = "";
					}
					//Enlever la fonction et � continuer � examiner l'expression
					else {
